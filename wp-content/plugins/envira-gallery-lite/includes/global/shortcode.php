...
class Envira_Gallery_Shortcode_Lite {
    ...
    public function shortcode( $atts ) {
        ...
        foreach ( $data['gallery'] as $id => $item ) {
            ...
            $gallery .= '<a href="' . $imagesrc . '" rel="lightbox">';

            $gallery .= '<img id="gallery-image-' . sanitize_html_class( $id ) . '" src="' . $imagesrc . '" alt="' . esc_attr( $item['title'] ) . '" title="' . strip_tags( html_entity_decode( $item['title'] ) ) . '" />';
